...
def found(input_path, EPC_str, past_estimate, N_start, reusability, sigma, den_thres):
    ...
    avg_rssi = [0.0]
    density_counter = [0]
    print("Initialised: ", past_estimate*180/pi, " degrees")
        ...
        if no_dist_known:
            for idx in range(N_particles):
                if np.random.uniform(0, 1) > 0.5:
                    ...
                    sign = -0.9
                poss_theta = np.random.normal(sign*pi/2, spread)
    ...
    if read_data():
        ...
        if (density_counter[-1] / (phase_time[-1]-phase_time[0])) > density_threshold:
            ...
            if abs(theta_estimation) < 15*pi/180:
                print("Target found at: ", theta_estimation * 180 / pi, " degrees")
                return [True, theta_estimation]
            else:
                print("Not close enough", theta_estimation * 180 / pi, " degrees")
                return [False, theta_estimation]
        else:
            print("No Target yet")
            return [False, 1000]
    else:
...
def target_finder(input_path, EPC_str):
    ...
    while not target_in_sight:
        target_in_sight, theta = found(input_path, EPC_str, theta, 100, 0.1, 0.3, 10)
        time.sleep(0.5)
    input_file = open(input_path, "w")
    input_file.close()
    return theta
