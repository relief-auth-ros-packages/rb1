...
#include "range_libc/vendor/lodepng/lodepng.h"
#include "range_libc/vendor/distance_transform.h"
#include "range_libc/includes/RangeUtils.h"
...
namespace ranges {
	...
	public:
      ...
      float py_f = 0.0;
			...
			while (t < max_range)
      {
				px_f = x0 + ray_direction_x * t;
				py_f = y0 + ray_direction_y * t;

        // px, py depend on the relative position of obstacle and robot
        if (px_f >= x0)
          px = ceilf(px_f);
        else
          px = floorf(px_f);
