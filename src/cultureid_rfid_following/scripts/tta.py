def track(input_path, EPC_str, past_estimate, N_start, reusability, sigma, den_thres):
    ...
    def create_particle_list():
        if no_dist_known:
            for idx in range(N_particles):
                new_particle = Particle(np.random.uniform(-pi/2, pi/2), 1.01)
            ...
            for idx in range(N_particles):
                ...
                new_particle = Particle(poss_theta, 1.01)
                particles.append(new_particle)
    ...
    def update_particles():
        ...
        avg_weight = 0
        for idx in range(particles.__len__()):
            avg_weight = avg_weight + particles[idx].weight
        avg_weight = avg_weight / particles.__len__()
        reliability.append(avg_weight)
        for idx in range(particles.__len__()):
            for j in range(N_offsprings):
                poss_theta = particles[idx].theta + np.random.normal(0, spread)
                if abs(poss_theta) > pi/2:
                    poss_theta = sgn(poss_theta)*pi/2 - sgn(poss_theta)*(abs(poss_theta) - pi/2)
                new_particle = Particle(poss_theta, particles[idx].weight)
                particles.append(new_particle)
        if reliability[-1] < 0.9:
            return False
        else:
            return  True
    ...
    def calculate_weight(theta_entry, curr_particle):
        ...
        if curr_particle.parent_weight != 1.01:
            curr_particle.set_weight((((pi-theta_offset) / pi) + curr_particle.parent_weight)/2)
        else:
    ...
    # #######################################################################################################
    class Particle:
        def __init__(self, theta_to_normal, weight_of_parent):
            self.theta = theta_to_normal
            self.weight = 0
            self.parent_weight = weight_of_parent
            ...
            if idx > 0:
                ...
                if consecutive_antenna_entries_counter < N and phase_time[idx] - phase_time[idx-1] < lost_phase_time_threshold:
                    ...
                    if phase_time[idx] - phase_time[idx-1] < lost_phase_time_threshold:
                        if phase_time[idx] - latest_estimation_time < 1 or latest_estimation_time == 0:
                            latest_estimation_time = phase_time[idx]
                            evaluate_particles([currA, currB])
                            latest_reliable = update_particles()
                            if latest_reliable and all_reliable:
                                last_reliable.append(phase_time[idx])
                            else:
                                all_reliable = False
                                lost_target.append(1)
                                lost_target.pop(0)
                                return
                        else:
                            lost_target.append(1)
    ...
    if read_data():
        ...
        if lost_target[-1] == 0:
            ...
            alt_theta_estimation = math.sin(theta_estimation)*4*pi*L/lamda
            alt_theta_estimation = sgn(alt_theta_estimation)*(-2)*pi + alt_theta_estimation
            if abs(alt_theta_estimation*lamda/(4*pi*L))<1:
                alt_theta_estimation = math.asin(alt_theta_estimation*lamda/(4*pi*L))
                if abs(alt_theta_estimation - past_estimation) < abs(theta_estimation-past_estimation):
                    theta_estimation = alt_theta_estimation
                    print("Switch")
            if density_counter[-1] > den_thres:
                print(180/pi*theta_estimation, " degrees")
                return [theta_estimation, avg_rssi[-1], reliability[-1]]
            else:
                print("Not dense enough")
                return [past_estimation, 0.0, 0.0]
        else:
            print("not reliable data")
            print("returning previous estimation: ", 180/pi*past_estimation)
            return [past_estimation, 0.0, 0.0]
    else:
        print("No data")
        return [past_estimation, 0.0, 0.0]
